...
void search(vector<int>& candidates, int curr, int target, int curr_sum, vector<int>& temp, vector<vector<int> >& res){
    if(curr_sum == target){
        res.push_back(temp);
        return;
    }
    if(curr_sum > target || curr == candidates.size()) return;
    search(candidates, curr+1, target, curr_sum, temp, res);
    temp.push_back(candidates[curr]);
    search(candidates, curr, target, curr_sum+candidates[curr], temp, res);
    temp.pop_back();
}
