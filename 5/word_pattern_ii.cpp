...
bool search(string& pattern, string& str, int p_sz, int s_sz, int p_curr, int s_curr, unordered_map<char,pair<int,int> >& m, unordered_map<string, char>& n){
    ...
    if(m.find(pattern[p_curr]) != m.end()){
        ...
        if(check(str, s_curr, temp.first, temp.second)){
            ...
            bool res = search(pattern, str, p_sz, s_sz, p_curr+1, s_curr+(temp.second-temp.first)+1, m, n);
            if(res) return true;
        }
        else return false;
    }
...
int main(){
    cout<<wordPatternMatch("ab", "aa")<<endl;
